 ...
 * @author Sven
 */
public class FastDigging extends MagePluginEvent {
    ...
    public boolean callPlayerInteractEvent(PlayerInteractEvent event, Integer level) {
        if (event.getAction() == Action.RIGHT_CLICK_AIR || event.getAction() == Action.RIGHT_CLICK_BLOCK) {
            ...
            ent.addPotionEffect(new PotionEffect(PotionEffectType.FAST_DIGGING, level * 12 * 100, 10));
            if (self) {
                pl.sendMessage(ChatColor.GREEN + "You got Fast Digging!");
            } else {
                pl.sendMessage(ChatColor.GREEN + "You gave Fast Digging to " + ent.getName());
                ent.sendMessage(ChatColor.GREEN + "You got Fast Digging from " + pl.getName());
            }
