...
public class Damage extends MagePluginEvent {
    ...
    public boolean callPlayerInteractEvent(PlayerInteractEvent event, Integer level) {
        if (event.getAction() == Action.RIGHT_CLICK_AIR || event.getAction() == Action.RIGHT_CLICK_BLOCK) {
            ...
            ent.addPotionEffect(new PotionEffect(PotionEffectType.INCREASE_DAMAGE, level * 10, 2));
            if (self) {
                pl.sendMessage(ChatColor.GREEN + "You got Incresed Damage for " + level * 10 + "s!");
            } else {
                pl.sendMessage(ChatColor.GREEN + "You gave Incresed Damage for " + level * 10 + "s to " + ent.getName());
                ent.sendMessage(ChatColor.GREEN + "You got Incresed Damage for " + level * 10 + "s from " + pl.getName());
            }
